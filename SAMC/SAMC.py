...
class InnerProductDecoder(nn.Module):
    """Simple decoder for prediction using inner product of embeddings."""
    ...
    def forward(self, z, zt=None):
        """
        Computes the inner product between embeddings.

        Args:
            z (torch.Tensor): The primary embeddings (N x D).
            zt (torch.Tensor, optional): The target embeddings (N x D). If None,
                                         computes inner product of z with its transpose.
                                         Defaults to None.

        Returns:
            torch.Tensor: The resulting similarity matrix (N x N).
        """
        if zt is None:
...
def thresholding(p, beta_1=0, beta_2=0.7):
    """
    Thresholding filter to identify datapoints with high-confidence cluster assignments.

    A sample is considered high-confidence if its maximum cluster assignment probability
    exceeds a certain threshold (`beta_2`).

    Args:
        p (torch.Tensor): The soft cluster assignment probability matrix (N x K).
        beta_1 (float): Lower bound threshold (currently unused, kept for compatibility).
        beta_2 (float): The confidence threshold over which an assignment is considered reliable.

    Returns:
        tuple: (high_confidence_indices, low_confidence_indices) as numpy arrays.
    """
    p_np = p.detach().numpy()
